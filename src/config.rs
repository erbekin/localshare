...
use actix_web::{Either, HttpResponse, http::StatusCode, http::header::ContentType, web};
...
pub fn get_404_page() -> actix_web::Route {
    web::get().to(async |app_state: web::Data<AppState>| -> Either<_, _> {
        match NamedFile::open_async(
            app_state
                .static_dir
                .join(PathBuf::from(StaticFile::NotFound)),
        )
        .await
        {
            Ok(f) => Either::Left(f.customize().with_status(StatusCode::NOT_FOUND)),
            Err(e) => {
...
pub fn get_upload_page() -> actix_web::Route {
    web::get().to(
        async |app_state: web::Data<AppState>| -> actix_web::Result<NamedFile> {
            match NamedFile::open_async(
                app_state.static_dir.join(PathBuf::from(StaticFile::Upload)),
            )
            .await
            {
                Ok(f) => Ok(f),
...
pub fn get_index_page() -> actix_web::Route {
    web::get().to(
        async |app_state: web::Data<AppState>| -> actix_web::Result<NamedFile> {
            match NamedFile::open_async(app_state.static_dir.join(PathBuf::from(StaticFile::Index)))
                .await
            {
                Ok(f) => Ok(f),
                Err(e) => {
